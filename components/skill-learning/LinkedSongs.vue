...
简谱中数字下方的横线（减时线/beam）表示时值缩短：
- 无横线 = 四分音符 = 1拍 → 直接写数字：3
- 1条横线 = 八分音符 = 半拍
- 2条横线 = 十六分音符 = 1/4拍

**括号规则：每个括号 = 1拍，括号内允许 2、3 或 4 个音符**

- 2个音 = 两个八分音符（各1/2拍）：(32)
- 3个音 = 混合节奏（1/4+1/4+1/2拍）：(234)  ← 极常见！
- 4个音 = 四个十六分音符（各1/4拍）：(5323)

⚠️ 3个音的括号是中文流行乐最常见的节奏型！
当连续3个音共享横线组（如前2个有2条横线 + 后1个有1条横线），必须放入同一个括号。
致命错误：把 (234) 拆成 (23) 4 → 这变成了2拍，节奏完全错乱！
...
附点音符（小圆点表示延长50%时值）：
  6.          附点四分音符 = 1.5拍（. 紧跟数字，无空格）
  (32) .      附点组 = 1.5拍（⚠️ . 与括号之间必须有空格！）
  (3.2)       括号内附点 = 附点八分+十六分共享1拍（3占3/4拍，2占1/4拍）
  ⚠️ 括号内的 . 表示附点节奏，不是高音标记！高音必须用 ' 标记
  ⚠️ 区分：数字正上方的圆点 = 高音 → 写 1'；数字右下角的小点 = 附点 → 写 1.
...
小节2：3 4 5 -（5 后有延音线连到下一拍）
小节3：0和3共享1条横线，然后4和3共享1条横线，然后2、3有2条横线+4有1条横线共享beam组，最后4和3共享1条横线后带附点
小节4：1' 6. 5 5. 1（1上方有高音点，6和5各带附点）

输出：
BPM=120
KEY=C
1 2 3 1 | 3 4 5 - | (03) (43) (234) (43) . | 1' 6. 5 5. 1 |

逐小节解读：
- 小节1：4个四分音符 = 4拍 ✓
- 小节2：四分+四分+四分+延长 = 4拍 ✓
- 小节3：(03)=1拍 + (43)=1拍 + (234)=1拍（3个音的混合节奏！）+ (43) .=1.5拍 = 4.5拍
  注：首拍 (03) 可能是前一小节的弱起，实际小节从 (43) 开始 = 3.5拍+附点补足
- 小节4：1'=1拍 + 6.=1.5拍 + 5=1拍 + 5.=无法凑满，说明可能节奏更复杂。这只是示意。
...
4. ❌ 把5个及以上的音塞进一个括号
   ...
   正确：(2352) (32) ← 分成1拍+1拍（具体看原谱横线分组）
   错误：(16165) ← 5个音
   正确：(1616) 5 或 (16) (16) 5 ← 看原谱横线怎么分
   注意：2、3、4个音的括号都是合法的，只有5个及以上才需要拆分
...
8. ❌ 混淆高音点和附点
   数字正上方/正下方的圆点 = 高低音，用 ' 或 , 标记
   数字右下角的小点 = 附点，用 . 标记
   错误：(1.2) 表示"高音1+2" ← 应写 (1'2)
   正确用法：(3.2) 表示"附点八分3 + 十六分2"共享1拍
   ⚠️ 括号内的 . 一律当附点处理！高音必须写 ' 不能写 .

9. ❌ 把3个音的混合节奏拆成2+1
   原谱：2̲̲ 3̲̲ 4̲（前2个有2条横线，后1个有1条横线，共享beam组）
   错误：(23) 4  ← 这变成了2拍！
   正确：(234)  ← 这是1拍（1/4+1/4+1/2）
   ⚠️ 这是最常见的转录错误，看到3个音共享横线组时一定要放在同一个括号
