 ...
 */
export async function initProgressForUser(db: any, userId: number) {
  const allWords = await db.select({ id: vocabWords.id }).from(vocabWords);
  if (allWords.length === 0) return;

  for (let i = 0; i < allWords.length; i += BATCH_SIZE) {
    const batch = allWords.slice(i, i + BATCH_SIZE).map((w: any) => ({
      userId,
      wordId: w.id,
 ...
 */
export function initProgressForUsersSync(tx: any, userIds: number[]) {
