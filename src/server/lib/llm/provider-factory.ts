...
export class ProviderFactory {
  ...
  static fromDbConfig(config: LlmProvider): ILlmProvider {
    const apiKey = config.apiKey ? decryptApiKey(config.apiKey) : null;
    ...
    switch (config.provider) {
      ...
      case 'claude-api':
        ...
        return new ClaudeApiProvider(config.modelName as ClaudeApiModel, apiKey);
      case 'ollama':
      ...
      case 'gemini':
        ...
        return new GeminiProvider(config.modelName, apiKey);
      default:
        throw new LlmError(
